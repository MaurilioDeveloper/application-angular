...
var DocumentComponent = /** @class */ (function () {
    ...
    DocumentComponent.prototype.downloadDoc = function (doc) {
        ...
        var document;
        if (doc.idDoc == 1) {
            document = this.appService.xFileDownload("documentationService", "printDocsFromSantander/" + this.getSimulation().id + "/" + 1 + "/", "CET", "pdf");
        }
        else if (doc.idDoc == 2) {
            document = this.appService.xFileDownload("documentationService", "printDocsFromSantander/" + this.getSimulation().id + "/" + 2 + "/", "CCB", "pdf");
        }
        else if (doc.idDoc == 3) {
            document = this.appService.xFileDownload("documentationService", "printDocsFromSantander/" + this.getSimulation().id + "/" + 5 + "/", "APROVACAO", "pdf");
        }
        else if (doc.idDoc == 4) {
            document = this.appService.xFileDownload("documentationService", "printFileFromS3/" + doc.link + "/", "CCB_CLAUSULAS", "pdf");
        }
        else if (doc.idDoc == 5) {
            document = this.appService.xFileDownload("documentationService", "printSPF/" + this.getSimulation().id + "/", "SPF", "pdf");
        }
        else if (doc.idDoc == 6) {
            document = this.appService.xFileDownload("documentationService", "printGAP/" + this.getSimulation().id + "/", "GAP", "pdf");
        }
        else if (doc.idDoc == 7) {
